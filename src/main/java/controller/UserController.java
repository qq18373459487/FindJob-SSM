...
public class UserController {
    ...
    @RequestMapping("SentInterviewEmail")
    public String SentInterviewEmail(String id,Model model,HttpSession session)
    {
        return  userService.SentInterview(id,model,session);
    }
    //文件的上传功能
    @RequestMapping(value="/uploadFile" ,produces="text/html;charset=utf-8" )
    public @ResponseBody String importPicFile1(@RequestParam MultipartFile file1) {
...
//            String floderName = fileBaseName + "_" + df.format(now);
            try {
                //创建要上传的路径
                File fdir = new File("C:\\Users\\曾康\\Desktop\\file");
                if (!fdir.exists()) {
                    fdir.mkdirs();
                }
                //文件上传到路径下
    ...
    public void download(@RequestParam(value="filename")String filename, HttpServletResponse response) throws IOException {
        ...
        String path = "C:\\Users\\曾康\\Desktop\\file"+filename;
        //获取输入流
