...
public class WorkServiceImpl implements WorkService {
    ...
    @Override
    public String SelectWorkByWorkType(String type, Model model, HttpSession session,String page) {
        int max=queryPageByWorkType(type);
        int   ReturnPage;
        try {
            ReturnPage=Integer.valueOf(page);
        }catch (Exception e)
        ...
        model.addAttribute("maxPage", max);
        //调用service层查询第几页的数据
        List<WorkList> list= queryWorkListPageByWorkType(ReturnPage,type);
        //绑定查询的结果到列表
        model.addAttribute("list",list);
        //绑定当前的页码
        model.addAttribute("page", ReturnPage);
    ...
    @Override
    public List<WorkList> queryWorkListPageByWorkType(int page, String WorkType) {
        int n = this.num;
        int m = n * (page - 1);
        return workMapper.queryWorkListByWorkType(m,n,WorkType);
    }

    @Override
    public int queryPageByWorkType(String WorkType) {
        int count = workMapper.queryPageByWorkType(WorkType);
        int page;
        if (count%this.num==0) {
            page = count/this.num;
        } else {
