            ...
            <div class="page-wrap">
                <table class="table table-bordered table-striped table-hover">
                    ...
                    <c:forEach items="${list}" var="company">
                        ...
                        <tr class="cen">
                            <td >${company.getId()}</td>
                            <td >${company.getComment_name()}</td>
                            <td >${company.getComment_email()}</td>
                            <td >${company.getComment_text()}</td>
                            <td >${company.getArticle_id()}</td>
                            <td >${company.getState()}</td>
                            <td>
                                <a title="删除" onclick="deleteComment(${company.getId()})">删除</a>
...
<script>
    $(".pagination").createPage({
        ...
        backFn:function(p)
        {
            window.location.href="http://localhost:8080/com_hnist_war_exploded/user/getComment?page="+p;
        }
    });
</script>
...
<script type="text/javascript">
    ...
    {
        alert("确定要删除吗？");
        window.location.href="http://localhost:8080/com_hnist_war_exploded/user/deleteComment?id="+id;
    }


    function updateState(id,state)
    {
        window.location.href="http://localhost:8080/com_hnist_war_exploded/user/updateCommentState?id="+id+'&&state='+state;
    }
